...
def makepkt(data, seqnum, checksum):

    packet = data
    packet = packet.append(seqnum)
    packet = packet.append(checksum)
...
def udt_send(sndpkt):
    clientSocket.sendto(sndpkt, (ServerName, ServerPort))

def rdt_rcv(rcvpkt):
    rcvpkt, clientAddress = serverSocket.recvfrom(PacketSize)
